...
class AttModule(nn.Module):
    ...
    def forward(self, x, f, mask):
        ...
        out = self.feed_forward(x) #temporal conv
...
class SFI(nn.Module):
    ...
    def forward(self, feature_s, feature_t, mask): #feature_s from（n,t,v) feature_t from (n,t,c)
...
class STI(nn.Module):
    def __init__(self, node, in_channel, n_features, out_channel, num_layers, SFI_layer, channel_masking_rate=0.3, alpha=1):
        ...
        self.conv_in = nn.Conv2d(in_channel, (num_SFI_layers+1)*8, kernel_size=1) #64->10
        self.conv_t = nn.Conv1d(node * 8, n_features, 1) #VC->64
        ...
        self.layers = nn.ModuleList(
            [AttModule(2 ** i, n_features, n_features, 'encoder', alpha) for i in 
                range(num_layers)]) #10 Attention
        self.conv_out = nn.Conv1d(n_features, out_channel, 1)

    def forward(self, x, mask):
        if self.channel_masking_rate > 0:
            x = self.dropout(x) #drop 0.3

        count = 0
        x = self.conv_in(x) #c=64->80
        feature_s, feature_t = torch.split(x, (len(self.SFI_layers)*8, 8), dim=1) #（n,80,t,v）->(n,72,t,v)+(n,9,t,v)
...
class Model(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.SP = MultiScale_GraphConv(13, in_channel, n_features, dataset)   #Multi-scale GCN
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor, joint_graph) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        x = self.SP(x, joint_graph) * mask.unsqueeze(3) # （n,c,t,v） spatial

        feature = self.STI(x, mask) #temporal
